...
PATTERNS = {
    ...
    # 密级标记（显式密级标签，非结构化敏感文本中的高置信格式）：【机密】/【绝密】/【秘密】/
    # 密级：机密 / 密级:绝密 / （机密）/ ［秘密］ 等。只覆盖「有固定包裹/前缀」的密级标签，
    # 纯语义的"内幕信息/未公开重大信息"无法靠正则可靠识别，不在此列（由触发壳上云前
    # 自查清单兜底）。匹配后整段标签整体脱敏，误报率低。
    "classification": re.compile(
        r"(?:"
        r"密级\s*[:：]\s*(?:机密|绝密|秘密)"              # 密级：机密 / 密级:绝密（整词）
        r"|(?:[【\[［(（][\s]*(?:机密|绝密|秘密|内部资料|内部文件|内参)[\s]*[】\]］)）])"
