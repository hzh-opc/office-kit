...
| 情形 | 行为 |
|------|------|
| **DESEN 已装**（或已装任意脱敏技能/工具） | 外发命令执行前**强制前置 `desen scan`**（`kit.py` 代码级门禁，扫真实输入文件 + stdin + URL）：命中敏感 → **先阻断（exit 3）+ 确认卡**，用户 `--confirm-raw` 确认并 `desen audit-log --decision raw` 留痕后放行；或选脱敏外发（`desen run` + `audit-log --decision desen` 全链路追溯）；无敏感 → 静默放行；扫描异常 → fail-safe 保守阻断。各组件单独调用（S4）时由 `skill_bridge.py` 的 confirm-or-block 确认闸口兜底 |
...
> 组件升级/修复（`upgrade`/`repair`）属「组件自更新」联网，非外发用户信息，不触发本铁律。`desen` 自身「脱敏副本上云」已有确认闸门 + 清单，是铁律的正确实现范本。表格云解析（sheetagent）等 **office-kit 之外的插件通道**由 `desen-trigger` + `desen-stop` Stop Hook 兜底，须在业务层约定「送云前先 `desen scan`」。
