...
**凡外发信息必先 `desen scan`，未扫即阻断。** 本铁律适用于一切把信息送出本机的动作，包括显式外发与隐性外发：

- **显式外发**：`tencent-doc`（Markdown→腾讯文档云端）、`summarize --mode cloud/hybrid`（摘要 brief 上云）、脱敏副本上云。
- **隐性外发**：`podcast --tts`（讲稿全文送 edge/openai/azure 第三方 TTS）、多语翻译（translation）、联网补全（search，query 可能含敏感原文）、网页抓取（WebFetch）、要点沉淀进知识库、识别稿外发。

执行规则（分「DESEN 已装 / 未装」两路）：

| 情形 | 行为 |
|------|------|
| **DESEN 已装**（或已装任意脱敏技能/工具） | 外发命令执行前**强制前置 `desen scan`**，未通过扫描即**阻断并提示**，不得静默放行；`kit.py` 已做代码级门禁，各组件单独调用（S4）时同样由 `skill_bridge.py` 强制「已装即必扫」 |
| **DESEN 未装**（用户未装/不愿装） | **不随意阻断任务**，改为**显式提醒**（"未检测到脱敏技能，本次外发未经完整脱敏，请自行确认是否含敏感信息"）+ **组件自带最小脱敏兜底**（本地 PII 预检/掩码），让用户在知情前提下继续 |

> 组件升级/修复（`upgrade`/`repair`）属「组件自更新」联网，非外发用户信息，不触发本铁律。`desen` 自身「脱敏副本上云」已有确认闸门 + 清单，是铁律的正确实现范本。
