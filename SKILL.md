...
## 外发安全政策（2026-09-04 细化版）

套件对外发动作按「用户是否主动发起、是否具有明显上云/分享意图」分两类处理，**不再强制脱敏、不再硬阻断**：

- **显式外发（用户主动发起、明显上云/分享意图）**：如 `tencent-doc`（Markdown→腾讯文档云端）、将本机文件同步/分享到网盘/飞书等。**不主动进行脱敏处理**，信息安全由用户与服务平台负责；门禁直接放行、不打扰。
- **隐性外发（非用户明显意图的上云/联网）**：如 `summarize` 的翻译/TTS/联网补全/知识库沉淀、识别稿外发等。**不强制脱敏**，但命中敏感信息时**显式提示**（warn），由用户自行决策，**不阻断**。

执行规则（分「显式 / 隐性+DESEN 已装 / 隐性+DESEN 未装」三路）：

| 情形 | 行为 |
|------|------|
| **显式外发** | 直接放行、不打扰；用户与平台对信息安全负责（套件政策：不主动脱敏） |
| **隐性外发 + DESEN 已装** | 执行前前置 `desen scan`，**命中敏感信息仅显式提示、不阻断**；`kit.py` 门禁按 `IMPLICIT_EXTERNAL` 仅提示 |
| **隐性外发 + DESEN 未装** | 不阻断，仅**显式提醒**（"未检测到脱敏技能，本次外发未经完整脱敏，请自行确认是否含敏感信息"）+ **组件自带最小脱敏兜底**（本地 PII 预检/掩码） |
| **环境变量 `OFFICE_KIT_SKIP_EXTERNAL_GATE=1`** | 显式跳过隐性外发的提示逻辑（与 security-scan 的 Skip 档一致） |

> 组件升级/修复（`upgrade`/`repair`）属「组件自更新」联网，非外发用户信息，不触发本政策。原「外发必扫 DESEN 铁律（未扫即阻断）」已于 2026-09-04 按用户拍板政策调整为「显式放行、隐性提示」——`desen` 脱敏能力仍可通过 `desen run` 由用户主动调用，未取消。
