...
PATTERNS = {
    ...
    # ── 密级/内部标记（2026-09-04 按「一般企业/单位不应接触国家秘密」政策拆分两类）──
    #
    # ① state_secret：法定国家秘密等级字眼（机密/绝密/秘密）。按《保守国家秘密法》，
    #    一般企业/单位依法接触不到国家秘密载体；若文档出现此类标记，多为「内部文件误
    #    用机密字眼」或来源异常，须**显式提醒用户确认**（不得静默脱敏后当普通字段上云）。
    #    只覆盖「有固定包裹/前缀」的高置信格式（方括号/书名号/圆括号/密级：前缀/★），
    #    降低误报；纯语义的"内幕/未公开重大信息"无固定格式、不在此列（触发壳自查清单兜底）。
    "state_secret": re.compile(
        r"(?:"
        r"密级\s*[:：]\s*(?:机密|绝密|秘密)"              # 密级：机密 / 密级:绝密（整词）
        r"|(?:[【\[［(（]\s*(?:机密|绝密|秘密)\s*[】\]］)）])"  # 【机密】/［秘密］/（绝密）
        r"|(?:★{1,3}(?:机密|绝密|秘密))"                 # ★机密 / ★★★绝密
        r")"
    ),
    # ② internal_mark：企业内部标签（内部资料/内部文件/内参）——非国家秘密，属企业正常
    #    内部敏感标记，维持脱敏、不触发国家秘密确认提醒。仅高置信包裹/限定形式，收敛误报。
    "internal_mark": re.compile(
        r"(?:"
        r"(?:[【\[［(（]\s*(?:内部资料|内部文件|内部文档|内参)\s*[】\]］)）])"  # 【内部资料】/（内参）
        r"|(?:内部(?:资料|文件|文档|编号)\s*[:：])"    # 内部资料：/内部编号：（限定前缀）
        r")"
    ),
}
