...
## v2.11.4 · 2026-09-12（上游反馈 F1：新增 --full-rules 完整场景化铁律注入）

- **新增 `--full-rules` 参数**（`install.py`）：写入**完整场景化常驻铁律**（「常驻执行规则：办公任务统一入口（场景化铁律）」触发清单 +「常驻执行规则：敏感信息外发检测（铁律）」四要素闭环），替代此前仅 4 行简版「任务前检测闸门」。以 `FULL_MARKERS` 任一命中去重（幂等）；与简版 `RULE_BLOCK` 并存无害，跨源去重逻辑不变。
- **动机**（office-kit 上游反馈 F1，P1）：`kit.py verify` 的 SOUL 检查关键词（「办公任务统一入口」+「敏感信息外发检测」）与简版注入内容不闭环——纯新机器上按 verify 提示跑 `bootstrap --inject-soul-rules` 后重跑 verify 仍警告，修复路径失效。office-kit `bootstrap.sh/ps1` 第 7 步已改为传 `--full-rules`。
- 治理防线补齐：注入内容现含场景化触发清单（防漏触发/误触发）与外发四要素（范围/动作/命中处理/云端回填），不再依赖部署提示词手工照抄模板。

## v2.11.3 · 2026-09-12（回灌安全加固：密级/内部标记识别 + stdin 管道输入）

本次为**回灌版本**：以下能力先前只存在于 office-kit 套件副本，**未同步回独立仓库**，导致走 S4「仅组件独立安装」的用户拿不到这些安全能力（见 2026-09-11 全面检查报告 P0-2）。本次回灌并单独立版。

- **新增：密级 / 内部标记识别拆两类**（`scripts/desen_rules.py`）：
  - `state_secret`——法定国家秘密等级标记的高置信格式（`【机密】/【绝密】/【秘密】`、`密级：机密`、`★机密`）。命中即输出**显式确认提醒**（请用户确认「确属国家秘密载体」还是「内部文件误用机密字眼」），掩码为 `[涉密标记]`；**不改变保密属性、不豁免保密责任**。
  - `internal_mark`——企业内部标签（`【内部资料】/【内部文件】/（内参）`、`内部资料：`），按普通敏感标记脱敏为 `[内部]`，不触发国家秘密确认提醒。
- **新增：`scan` / `run` 支持 stdin 管道输入**（输入参数写 `-`，见 `_resolve_stdin`）——服务「联网搜索 / 发邮件」等**纯文本外发**场景的快速检测，无需先落盘。
- **SKILL.md 补强**：新增**边界声明**（本技能面向 PIPL 个人信息与企业内部商业秘密，**不处理法定国家秘密**；最高保密场景应隔离运行）与**行业差异化留存期**提醒（证券/基金留痕依规 ≥ 20 年；审计底稿依《会计师事务所数据安全管理暂行办法》核心 ≥ 3 年 / 重要 ≥ 1 年）。
- **`install.py`**：`resolve_venv_dir` 增加「复用已部署生产副本」档位（`OFFICE_KIT_ROOT/.venv` → `~/office-kit/.venv`），套件在场时依赖统一装进套件 venv，杜绝「一份依赖多个 venv」。

## v2.11.2 · 2026-09-04（对话内展示约定 + 映射表路径收敛——落地交互展示反馈 7 项）

- **安全：映射表（重识别钥匙）路径不再回显**（P0-②）：`_refresh_index` 生成的 `成果索引.md` 与 `status` 回显中，映射表条目以「🔒 已加密封存（留本地，不展示路径）」替代绝对路径；真实路径仅写入本地 `成果索引.json`（供本机复核，不默认回显）。收敛「对话内不回显映射表路径」红线（此前只约束审计卡片，漏掉成果索引 / status 回显两条旁路）。
- **审计文档增「审计摘要」置顶段**（P2-①）：`audit` 产物顶部新增风险自评 / 外发范围 / 结论摘要，非专业人员一屏内掌握「这次脱敏安不安全」；九节细节保留供审计复核。
- **审计文档增「脱敏前后对照示例」**（P2-②）：以脱敏后形态示例（`138****8000` / `4*****************` / `张**`）说明「保留字段语义、可统计不可还原身份」，精确原值↔令牌对照仍只存加密映射表、不展示。
- **SKILL.md 增「对话内展示约定」节**（P0-① / P1-①②③）：固化「脱敏审计卡片 / 敏感信息清单表 / 外发边界清单 / 过程文件暂存+附路径」四类对话内结构化展示约定，并上提「映射表及其路径对话内不展示」安全红线；同步修订「动作二」模板中映射表路径行。
