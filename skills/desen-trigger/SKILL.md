...
- 未命中 → 直接执行，零额外负担。
- **密级标记（desen v2.11+ 可正则识别）**：`【机密】`/`【绝密】`/`密级：机密`/`★机密`/`[内部文件]` 等显式密级标签会命中 `classification` 并脱敏为 `[密级]`。**纯语义的「内幕信息 / 未公开重大信息」无固定格式、无法正则识别**，desen scan 会返回「未发现」——此类须由 Agent 按下方「财务/审计/投研上云前自查清单」人工把关，不得误以为「未发现」即「无敏感」。
